model_4 <- function(price_zone){
  ...
  ######### MODEL ##########
  regression = lm(Price ~ `Peak (08:00-22:00)` + Winter + Spring + Summer + Lambda + Price_previous, data_train)
...
##################
descripiption  = paste("MSE = ", model_4_1$MSE[1], ". Mean relative error = ", model_4_1$Mean_relative_erro[1], sep = '')
plot(model_4_1$Price, type = 'l', col = 'blue', lty = 1,
     axes = FALSE, ann = FALSE)
par(new = TRUE)
plot(model_4_1$Predicted_price, type = 'l', col = 'red', lty = 2,
     main = 'Prediction, model 4. Price zone 1', sub = descripiption,
     xlab = 'Hour', ylab = 'Price, RUB', font.lab=2 )
grid(nx = length(model_4_1$Price), ny = 10, col = "lightgray", lty = "dotted",
...
###################
descripiption  = paste("MSE = ", model_4_2$MSE[1], ". Mean relative error = ", model_4_2$Mean_relative_erro[1], sep = '')
plot(model_4_2$Price, type = 'l', col = 'blue', lty = 1,
     axes = FALSE, ann = FALSE)
par(new = TRUE)
plot(model_4_2$Predicted_price, type = 'l', col = 'red', lty = 2,
     main = 'Prediction, model 4. Price zone 2', sub = descripiption,
     xlab = 'Hour', ylab = 'Price, RUB', font.lab=2 )
grid(nx = length(model_4_1$Price), ny = 10, col = "lightgray", lty = "dotted",
     lwd = par("lwd"), equilogs = TRUE)
legend('topright', c('Actual data', 'Predicted data') ,
